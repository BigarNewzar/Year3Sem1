...
class DefaultErrorStrategy(ErrorStrategy):
    ...
    #
    # @param recognizer the plparser instance
    #
    ...
    # {@inheritDoc}
    #
    # <p>The default implementation resynchronizes the plparser by consuming tokens
    ...
    # A single mismatch token or missing token would force the plparser to bail
    ...
    # </pre>
    #
    # input with an extra token between members would force the plparser to
    # consume until it found the next class definition rather than the next
    # member definition of the current class.
    #
    # <p>This functionality cost a little bit of effort because the plparser has to
    ...
    # and use the plparser's {@link TokenFactory} to create it on the fly. The
    ...
    # the plparser returns from the nested call to {@code expr}, it will have
    # call chain:</p>
    #
    # <pre>
    ...
    # from mismatched input. If this method returns null, the plparser and error
    ...
    #  action in the plparser or tree plparser works. It simply creates
    ...
    #  rule invocation, the plparser pushes the set of tokens that can
    ...
    #
    #  So, you can see that the plparser walks up the call chain looking
...
class BailErrorStrategy(DefaultErrorStrategy):
    ...
    # Make sure we don't attempt to recover inline; if the plparser
