...
#
# <p>The lexer and plparser that you pass into the {@link ParseTreePatternMatcher}
# constructor are used to parse the pattern in string form. The lexer converts
# the {@code <ID> = <expr>;} into a sequence of four tokens (assuming lexer
# throws out whitespace or puts it on a hidden channel). Be aware that the
# input stream is reset for the lexer (but not the plparser; a
# {@link ParserInterpreter} is created to parse the input.). Any user-defined
# fields you have put into the lexer might get changed when this mechanism asks
# it to scan the pattern string.</p>
#
# <p>Normally a plparser does not accept token {@code <expr>} as a valid
# {@code expr} but, from the plparser passed in, we create a special version of
...
class ParseTreePatternMatcher(object):
    ...
    # the tree patterns. The plparser is used as a convenient mechanism to get
    ...
    # syntax used by the tree pattern plparser.
    #
