...
#
# A {@link Token} object representing an entire subtree matched by a plparser
# rule; e.g., {@code <expr>}. These tokens are created for {@link TagChunk}
# chunks where the tag corresponds to a plparser rule.
#
...
class RuleTagToken(Token):
    ...
    #
    # @param ruleName The name of the plparser rule this rule tag matches.
    # @param bypassTokenType The bypass token type assigned to the plparser rule.
    ...
    def __init__(self, ruleName:str, bypassTokenType:int, label:str=None):
        ...
        self.channel = Token.DEFAULT_CHANNEL # The plparser ignores everything not on DEFAULT_CHANNEL
