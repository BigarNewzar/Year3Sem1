...
# memory. The goal is not to create the world's fastest plparser anyway. I'd like
...
#
# <p>
# All instances of the same plparser share the same decision DFAs through a
...
# syntax error. For maximum speed, Sam starts the plparser set to pure SLL
# mode with the {@link BailErrorStrategy}:</p>
#
# <pre>
# plparser.{@link Parser#getInterpreter() getInterpreter()}.{@link #setPredictionMode setPredictionMode}{@code (}{@link PredictionMode#SLL}{@code )};
# plparser.{@link Parser#setErrorHandler setErrorHandler}(new {@link BailErrorStrategy}());
# </pre>
#
# <p>
...
class ParserATNSimulator(ATNSimulator):
    ...
    def __init__(self, parser:Parser, atn:ATN, decisionToDFA:list, sharedContextCache:PredictionContextCache):
        ...
        #  plparser/atnsim object because it can only handle one input at a time.
    ...
    def adaptivePredict(self, input:TokenStream, decision:int, outerContext:ParserRuleContext):
        ...
        try:
            if dfa.precedenceDfa:
                ...
                # plparser precedence, and is provided by a DFA method.
            ...
            if s0 is None:
                ...
                if ParserATNSimulator.debug:
                    print("predictATN decision " + str(dfa.decision) +
                                       " exec LA(1)==" + self.getLookaheadName(input) +
                                       ", outerContext=" + str(outerContext));#outerContext.toString(self.plparser.literalNames, None))
    ...
    # the plparser. Specifically, this could occur if the <em>only</em> configuration
    # capable of successfully parsing to the end of the decision rule is
    # blocked by a semantic predicate. By choosing this alternative within
    # {@link #adaptivePredict} instead of throwing a
    # {@link NoViableAltException}, the resulting
    # {@link FailedPredicateException} in the plparser will identify the specific
    # predicate which is preventing the plparser from successfully parsing the
    ...
    # @param outerContext The is the \gamma_0 initial plparser context from the paper
    # or the plparser stack at the instant before prediction commences.
    #
    ...
    # the lookahead (but not plparser) consumes a token.
    #
    ...
    def reportAmbiguity(self, dfa:DFA, D:DFAState, startIndex:int, stopIndex:int,
        ...
        if ParserATNSimulator.debug or ParserATNSimulator.retry_debug:
#			ParserATNPathFinder finder = new ParserATNPathFinder(plparser, atn);
#			int i = 1;
#			for (Transition t : dfa.atnStartState.transitions) {
#				print("ALT "+i+"=");
#				print(startIndex+".."+stopIndex+", len(input)="+plparser.getInputStream().size());
#				TraceTree path = finder.trace(t.target, plparser.getContext(), (TokenStream)plparser.getInputStream(),
